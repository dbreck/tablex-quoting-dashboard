...
**Repos:** tablex-site `main` = `c7e01ab` (prod ● live-verified, auto-deploys from `main`). Dashboard `main` = `49d7480` (Vercel hub `danny@clearph.com`, repo `github.com/dbreck/tablex-quoting-dashboard`).

**7/19: /resources REAL LIBRARY SHIPPED** (`c7e01ab`, live-verified — memory [[project-resources-library]]): 63 PDFs curated from the legacy WP site (`~/Sites/tablex-og` public /pricing/ + /brochures/ pages) into new public Supabase bucket `resources` (tablex-site project; catalog `src/data/resources.ts` — every tile maps 1:1 to a real file). Brochures (Look Book + 10 series + 8 product + 8 option sheets) · price lists (Jan 5 2026 LIST book, complete + 19 sections, **un-gated** — public on legacy site; dealer NET stays unpublished) · Installation & Care (renamed; 9 install + 6 care + 2025 Warranty) · CAD reframed honestly (ProjectMatrix + MRL external doors, no in-house library — legacy had none). StatStrip fabrications killed (counts now derived). New book editions → NEW dated bucket folder, never overwrite. Danny-review flags: price-list un-gating + freight-zone tables render dollar figures (shipping policy, carried from legacy page). Follow-up: wire `/dealer/downloads` to the same catalog.
