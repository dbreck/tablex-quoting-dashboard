...
- **Gotchas:** Playwright MCP `filename` screenshots vanish — omit the filename (image returns inline) · the stale `next start` on :3000 was killed for the build (`next dev` uses `.next/dev`, `next build` overwrites `.next`) · a Workflow critique agent can stall ~30 min and retry — check `journal.jsonl`.
