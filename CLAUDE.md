...
## Current state (2026-09-08 PM) — NEXT SESSION = SELF-QUOTE BUILD (P-3) → FIND-A-REP → SHIP → ARCHIVE TEST QUOTES (tablex-site `main`=`e0a8024`, NOT PUSHED; :3100 server STOPPED)

**Danny approved every decision in `tablex-site/docs/plans/self-quote.md` (committed). ⚠ HOLD: the change-order hours went to Richie (Gmail draft `r6469758859351741202`, 11 items / 36h, Danny sends) — until approved, do NOT tell Brian/Mark that freight, SIF or the outdoor filter are built; Danny's sent reply framed SIF as future.** Brian's rep listing arrived: dashboard `docs/TableX Sales Rep Listing January 2026.xlsx` (11 groups, ~35 people, free-text territories, Verified flags) — Find-a-Rep is unblocked. Agreed order for the next session: (1) self-quote v1 · (2) Find-a-Rep from the listing · (3) ship the local commits · (4) archive TX-2026-0013…0021 · (5) small rep-demo items (P-8, P-9, P-4, D-3).

## Current state (2026-09-08 AM) — FREIGHT + SIF + OUTDOOR POWDER + LAST LAMINATE GAPS BUILT (tablex-site `8c5a131` + docs commit, LOCAL, NOT PUSHED)

**Brian and Mark answered the 9/08 "three inputs" email the same morning; all three rep-demo items (P-1 freight, P-2 SIF, F-5 outdoor) plus Neal's textures went in one day. Read memory [[project-freight-sif-outdoor-2026-09-08]].** Dashboard `/project/rep-demo` P-1/P-2/F-5 checked off with notes (author "Claude Code (9/08 build)"). Reply draft to Brian+Mark in Gmail (Cinder? exactly-$3K rule? order minimum? 20-20 manufacturer code?) — **send owed**.

- **P-1 Freight:** Brian's image = the table ALREADY in the active book (`price_book_freight_zones`, r4). New nullable `quotes.ship_to` (migration `quote_freight_v1`, applied via MCP); submit form REQUIRES a ship-to (states + the book's `MI-UP`/`NYC` tokens + DC/OTHER); `src/lib/pricing/freight.ts` (pure, 8 tests) resolves zone + bracket by LIST subtotal, `freight-lookup.ts` reads the active book; `issueQuote` derives the row (`withFreightCharge`, idempotent like oversize) → PDF "Ship to" meta + charge row, emails, desk preview, dealer/rep pages. Zone 5 / unlisted / no destination = **$0 "quoted separately" row** (never omitted silently); desk "Freight override ($)" on send → "Freight (quoted)". Exactly $3,000 takes the higher bracket. No order minimum in the table (asked).
- **P-2 SIF:** `GET /api/quote/[id]/sif` (issued-only, PDF's gate) from the published spec; charges as records; "Download SIF" beside the PDF on dealer + rep. **`MC=TBLX` is a PLACEHOLDER** until Mark names the 20-20/CET code. Mark has no sample (CET extension generates them).
- **F-5 Outdoor:** 22/23 of Brian's list matched by exact name → `outdoorRated`; `/finishes/powder-coats?outdoor=1` filter + inspector row; Element page names the count. **"Cinder" is not in the 40-color roster** — asked Brian.
- **Laminates:** Neal's Drive drop (`External Share/TableX/Assets/Finishes`, 65 color maps) closed the last gaps: Nordic Linen + Ashwood Beige + Hazel Walnut sheets **scale-calibrated** (Neal's 6750px frame ≈ 132 in, measured by grain-line period against 3 laminates we hold both sources for) and a Pressed Linen chip (Classic Linen donor recolored). `CHIP_MISSING` is empty.
- **Verified:** `pnpm build` green; 22 node tests; prod-mode walk on TX-2026-0020 (GA → "Freight (Zone 2, under $3K list) $300"; override re-send → "Freight (quoted) $485"; PDF/desk/dealer/SIF agree). TX-2026-0018 re-submitted with ship-to Indiana (routed to the desk, line unpriced). The :3100 prod server was stopped at shutdown; rebuild + `pnpm start -p 3100` to eyeball again. A stray magic-link email to danny@clearph.com came from my login attempt (ignore).
- **Gotchas:** the login form is magic-link by default — click "sign in with a password" first, or Playwright emails a link · `browser_run_code_unsafe` has no `require` (fetch signed URLs with curl instead) · zsh: quote `--include=*.ts` globs.

## Current state (2026-09-07) — EVERYTHING SHIPPED: tablex-site `main`=`59520c6` (`959028e` = code, 19 commits, deploy `q10ytf81z` ● Ready; `59520c6` = Codex closeout docs committed + stale-bullet fixes: realism "pending merge" → merged/live, Typekit `ucn5jze` → `juc1jwq`; NEW root `STATUS.md` = Codex's handoff file — Danny to rule keep-vs-fold-into-CLAUDE.md) · MCP FIXED · SCHEMA SYNC VERIFIED

- **Shipped in one push (`e08b928..959028e`):** the 15 rep-demo commits (9/06 build) + Codex's 3-commit `/products` scroller first-paint fix (`98511d6`→`c285463`: earlier mount, concurrent preloads, offscreen warm composite, white-table poster) + home scroller pacing/clip (`959028e`). Combined `pnpm build` green (98 pages) before push. Both branches deleted, worktree removed. Danny's `next dev` on :3000 is now on `main`.
- **`supabase-tablex` MCP "Connection closed" = corrupted npx cache missing peer-dep `zod`** — `rm -rf ~/.npm/_npx/53c4795544aaa350` + `/mcp` reconnect; token was fine. Memory [[reference-supabase-tablex-mcp-connection-closed]]. Verified via MCP that the 9/06 direct-pg schema (`20260906120000 rep_demo_quotes_v1`) matches live 1:1 and follows the repo convention (no migration files on disk — the `statements` column IS the record; no generated DB types exist).
- **Home 3D scroller pacing:** `SECTION_VH` 500→900, beats/phases/camera keys authored in vh-after-pin via `at()` in `choreography.ts`, all timing exactly 2× (tune the vh numbers, or `SECTION_VH` alone); finale Color row un-clipped (`py-1.5` on the Color `ChipRow`; the Fold body is overflow-hidden). Memory [[project-home-spex-scroll-3d-2026-09-05]] (9/07 block).
- **Still owed (unchanged):** Brian inputs draft `r-8976359271317901134` (send) · Mark 01↔30 Foundation alias · archive test quotes TX-2026-0013…0021 · D-3 dark ground · F-3 hero video · Codex-edited tablex-site `CLAUDE.md`/`AGENTS.md` review.

## Current state (2026-09-06 PM) — REP-DEMO FINDINGS BUILT IN ONE ULTRACODE DAY (tablex-site `main`=`03c43a0`, 15 commits, 48 files, build green, NOT PUSHED — Danny eyeballs :3000 then "ship it")

**Read memory [[project-rep-demo-build-2026-09-06]] + brief `tablex-site/docs/plans/rep-demo-findings-build-2026-09-06.md`.** Dashboard `/project/rep-demo` check-offs + notes posted (author "Claude Code (9/06 build)"). Vault log `2026-09-06-tablex-quoting-dashboard-rep-demo-findings-build`.

- **Schema APPLIED to prod via direct pg** (`supabase-tablex` MCP was down; the claude.ai Supabase connector sees only the personal org): `20260906120000 rep_demo_quotes_v1` — `quotes.expires_at/po_number/po_path/po_uploaded_at/revision/charges/revising_at/priced_source` + `quote_revisions` (RLS via parent). Tooling: `scratchpad/dbq.mjs` (pnpm-store `pg` + `DATABASE_URI`).
- **Built + verified (6 Playwright/review lanes on a prod-mode build):** F-1 line editing (qty/remove/replace-in-SpeX via `?quote=&line=`, thumbnails) · F-2 Foundation (real cause = CAD per-shape base union vs one base per size; book prices Foundation as **01 Series** — ASK MARK for the 30→01 alias) · R-1 instant pricing (`quote-issue.ts` = ONE issue path for auto + desk; lift gate $125 checkbox, oversize skid $300 auto; **only Core-laminate tops auto-issue — Select/Luxe/solid-surface + placeholders route to the desk until the PL-master +15/+35% adder exists**; desk-priced re-submits stay with the desk) · R-2 revisions (Edit quote → revising → rev N+1, history on 3 surfaces, dealer lock) · R-4 PO number + file on accept · R-5 30-day validity (end-of-day ET; accept refused after) · R-6 images (portal + PDF toggle) · R-7 (no change; /dealer/pricing stale claims cleaned) · R-8 tier placeholders + accept gate + resolve (D-2 default tier; no adder → same price today).
- **Owed:** Danny ship-it · Brian inputs (Gmail draft `r-8976359271317901134` cc Mark: F-5 outdoor list, P-1 freight table, P-2 sample SIF) · Mark 01↔30 · D-3 dark ground · F-3 hero video (human) · archive test quotes TX-2026-0013…0021 · pre-existing `Toast.tsx` lint error unchanged.
- **Gotchas:** one `next dev` per checkout (Next 16 lock) — prod-mode verification = `pnpm build` + `pnpm start -p 3100` alongside dev · Turbopack refuses symlinked `node_modules` in worktrees · Playwright shared tab → `browser().newContext()` per agent · `eslint .` lints `.vercel/output` — use `eslint src`.

## Current state (2026-09-06) — HOME FOLD 3 = SCROLL-DRIVEN 3D SPEX STORY, SHIPPED TO PROD (tablex-site `main`=`22d5887`) + CODEX/ASTRA'S OWN TAKE ON /products

**9/06 EOD STATE: tablex-site `main`=`e08b928` = Codex PR #7 "SpeX Studio realism, inspection, and presentation" ff-merged + pushed (prod deploy polled at hand-off; Danny restarting the session next).** Verified before merge: 67 files / 4 commits, tsc + eslint clean, `pnpm build` green, its 13 node tests pass (`node --import tsx --test src/components/spex/*.test.ts`), all three studio-sharing surfaces mount + render with zero console errors (`/spex-studio/ultra`, `/spex-studio/foundation`, `/products#spex-astra`, home fold). What it did: NEW `RealismRig.tsx` (neutral key + reflection cards, still a local 256px cubemap, no HDR fetch), `laminate-surface.ts` (Wilsonart finish-suffix → roughness, brand+item number now reach the material recipe; Formica/unknown unchanged), powder coat as dielectric (metalness 0, softer orange-peel), tight contact-shadow pass limited to 24cm above the floor, `StudioCamera`/`camera-views.ts` (Perspective/Top/Front/Underside views), `StudioControls`, `FinishStudies` + `studio-export.ts` (inspection/presentation tools in the configurator), `FittedAccessories`/`accessory-layout.ts`. **`visuals.ts` now has `studio2` (the configurator DEFAULT); `?visuals=studio` = the August rig, `?visuals=classic` = original. `isStudioVisuals()` is now "not classic".** Marketing surfaces (home fold, Astra) are isolated via `FinishRealismContext` (default false) so their materials/clipping bindings are untouched. Reports + before/after captures: `docs/reports/2026-09-06-spex-studio-realism.md`, `…-presentation.md`, `docs/architecture/spex-studio.md`; Codex also edited tablex-site `CLAUDE.md` + `AGENTS.md` (review those next session). Rollback = `DEFAULT_SPEX_VISUALS` back to `"studio"`.

**Earlier 9/06:** Astra shader-lifecycle fix (`f0d1ab1`, deploy `f4g5fzd36` ● Ready) closed the GL_INVALID_VALUE warnings. My 5-point addendum to the realism handoff doc (now committed in PR #7).**

**9/06: BOTH ARE IN PRODUCTION.** Danny loved round 2 ("You rock. I love it."), had me ship it (`main`=`9a0ef19`, deploy `581f68sx5` ● Ready), then ran OpenAI Codex (model "Astra") against the brief `tablex-site/docs/plans/astra-home-spex-brief.md` to build ITS OWN interpretation for comparison. Astra put its section on **`/products`** (`#spex-astra`, before the SPACES band; folder `src/components/home/spex-scroll-astra/` + `src/lib/home/spex-sequence-astra.ts`, plan `docs/plans/home-spex-scroll-astra.md`): 5 chapters, opaque per-model render targets dissolved by a compositor, big camera moves, no interactivity. Merged ff to `main`=`22d5887` and pushed (deploy pending at hand-off). It touched nothing shared; tsc/eslint/build clean; one noise item: ~24× `GL_INVALID_VALUE: glGetProgramiv` warnings from its shader precompile/dispose path (harmless, ask Codex to clean up). Danny's next: "tweaks in our next section".

**Danny's ask (9/05, then away for hours): replace the SpeX screenshot fold with an Apple-scroller 3D table — copy scrolls up and locks, table rises/rotates/zooms into place, then morphs tops/finishes/bases, even into another table, "so you can test all the options". DONE end-to-end; read memory [[project-home-spex-scroll-3d-2026-09-05]] + `tablex-site/docs/plans/home-spex-scroll-3d.md`.**

- **What it is:** 500vh section, sticky 100svh stage, GSAP ScrollTrigger SCRUB (no pin) bridged to the page Lenis → one smoothed progress number drives the R3F camera orbit, the copy entrance, captions, and the CTA. 10 beats, every one a REAL native GLB resolved server-side through the SpeX pipeline (`src/lib/home/spex-sequence.ts` throws at build otherwise): Foundation rect → round → racetrack → rect (clipping-plane wipes) → Handspun Slate → Skyline Walnut → Pinnacle Walnut (opacity fades) → Satin Silver base → Ultra U-Leg → Elite V-Leg (floor-up wipes on the same 36×60 top) → finale "your turn" chips (6 tops · 6 bases · 5 colors, timed cross-overs) whose CTA deep-links the exact config. Captions = canonical spec rows + real configuration code. Files: `src/components/home/spex-scroll/{choreography,engine,SpexScrollCanvas,HomeSpexScroll}` · `TableModel.tsx` exports `classifyMesh`/`prepareNativeScene` · 512px orange-peel normal.
- **Verified on dev:** 1600×1000 · 1440×800 · 390×844 · reduced motion (collapses to the old two-column spotlight, no canvas) · drag-to-orbit · chips · `pnpm build` green, `/` still static · tsc + eslint clean.
- **Round 2 (9/05 PM, Danny's recording: "transitions between tables"):** the clip wipe ghosted two tables at once → replaced with `morph` (procedural slab in `outline.ts` morphs the top outline from real dims while bases swap) + `swap` (old base sinks into the floor, new one rises, sequential, floor-clipped; top floats) + shadow crossfade around a midpoint re-bake. Verified forward, reverse, and on finale picks; `40fbdc3`→ round-2 commit pushed, prebuilt preview redeployed.
- **Gotchas burned:** three.js bakes `material.transparent` into the shader (OPAQUE define) — compile stage materials transparent ONCE, animate only opacity/depthWrite, or a fade is a hard cut · the React Compiler lint pushes every three mutation into a plain-TS engine class (lazy `useState`, not `useRef`) · inline `style` heights beat `motion-reduce:` variants · **React StrictMode double-invokes `useMemo`** — clone before re-parenting meshes, bind clipping planes in `register()`, or the kept handle's planes are silently off the materials · **Vercel PREVIEW env lacks `DATABASE_URI` → every branch preview of tablex-site errors** (Payload); preview = `vercel pull --environment=preview` + `vercel build` + `vercel deploy --prebuilt --scope dbreckxs-projects`.
- **Next:** Danny eyeballs the prebuilt preview → "ship it" = ff-merge the branch to `main` → push. Tuning lives in `choreography.ts` (windows/CAMERA_KEYS) + `spex-sequence.ts` (beats/chips). Skipped for v1: wipe seam line, idle drift, touch drag, mobile chips. Local dev server may still be on :3000.

## Current state (2026-09-04 PM) — SPEX LOCKING CASTERS SHIPPED FROM TABLEX'S OWN CAD (tablex-site `main`=`80ac1ea` pushed 9/04 EOD, deploy `tablex-site-10idg1ihk` ● Ready, live-verified on the alias; local branch `feat/spex-casters` merged, deletable)
...
- **The VertiGO caster is now `glb-models/accessories/acc_caster-lc.glb`** — one of the four twin-wheel casters baked into `74/tc1830h1524.glb` (⚠ `SHAPE_META`: `TC` = "Rectangular", `RT` = "Racetrack Oval"), extracted with trimesh (archive + script: `~/Documents/Clear ph/Clients/TableX/3D Files/Casters/`). Same convention as the Dekko units: origin = mating point (stem top), body z ≤ 0.
- **Placement is derived from the base's own geometry** (`src/lib/spex/base-feet-core.ts` pure + `src/components/spex/base-feet.ts` three adapter, WeakMap per raw useGLTF scene): every base in the CAD set ends in glide-pad bodies on the floor → one caster per pad cluster (disc bases get five automatically). Pads hide (geometry CLONES, never the shared cache), base lifts by `caster height − structure underside`, Villa/grommets ride up. Bases already on casters (8 VertiGO H + 2 nesting `…tt32ne`) render unchanged; < 3 feet → nothing (Gannet plates, 4 no-base files, single generic pedestals). Overlay drops the nominal `× 29″H` while casters are selected.
- **Detector rules that matter (all measured):** components by manifold edges (exactly two faces per edge — position-welding and vertex-adjacency both fused pucks into foot bars on `08/bt3084t26`) · pad ≤ 70 mm plan, ≤ 120 mm tall, touches floor ≤ 3 mm or "floating puck" ≤ 12 mm up & ≤ 15 mm tall (4 CAD files float a pedestal) · cluster 60 mm · wheel = floor-touching disc on edge 35–80 mm · padTop = min(pad top, underside of structure over the foot) because generic-base stems run up INTO the bar.
- **Acceptance test:** `pnpm tsx scripts/audit-base-feet.ts <dir> [--json] [--components]` over a bucket mirror (1,798 files): 4 feet ×1,257 · 6 ×132 · 8 ×62 · 5 ×34 · multi-disc/drum ×250 · baked casters ×10 · gated (0–2) ×40. Re-run it before touching thresholds.
- **Pre-existing bug fixed in `ModelPreloader`:** drei's ARRAY form `useGLTF([table, ...acc])` keys the R3F cache on the whole tuple, so single-URL display calls (Villa's `useGLTF(url)`) were never warm and could suspend the shared boundary. Now one single-URL `<WarmGltf>` per GLB; accessory failures swallowed. Casters render INSIDE TableModel (join the camera fit) under their own no-onError boundary; yaw via a PARENT group (yawing the −π/2-rotated object tips it).
- **Open for Mark/Brian (no code):** twin/multi-pedestal/drum bases render 6–24 casters vs the chip's "set of four" (price unit) · VertiGO H/nesting already on casters yet `-LC` stays selectable at $111 (book has base-specific caster rows $138/$166 the flat corroboration can't see) · Foundation CAD code 30 vs book "01 Series" = no option-charge corroboration.
- **Next:** Danny eyeballs live (`/spex-studio/vertigo?shape=RT&size=3660&base=T&acc=casters-lc`, `…shape=RD&size=30&base=D…`, `/spex-studio/foundation?shape=TC&size=3660&base=TT&acc=casters-lc`, casters + Villa + grommets) · email Mark the two caster-pricing questions above · nothing else pending on this arc.
- **Gotchas:** Playwright MCP `filename` screenshots vanish — omit the filename (image returns inline) · the stale `next start` on :3000 was killed for the build (`next dev` uses `.next/dev`, `next build` overwrites `.next`); no local server left running · a Workflow critique agent can stall ~30 min and retry — read `journal.jsonl` rather than waiting for the notification · a chained `cmd; echo; grep -c` background task reports exit 1 when grep counts 0 — check the log before calling a build failed.

## Current state (2026-09-04) — FEEDBACK BOARD SWEPT 30 → 8 OPEN (tablex-site `main`=`58c6e5f` pushed, 10 commits; Danny verifying visuals himself)

**Read memory [[project-board-sweep-2026-09-04]].** 22 cards → `review` with comments (as danny@ feedback user); 8 stay open, all photography/decision-blocked (#302 #303 #304 #305 #315 #372 #389 #392). Method: Workflow of 7 worktree-isolated implementers + audit + 9 verify-shipped → cherry-picks → build → Workflow of curl/Playwright/adversarial-diff verification → fixes → rebuild → push. Vault log `2026-09-04-tablex-quoting-dashboard-feedback-board-sweep`.

- **Shipped:** "Table Xpress" (space) everywhere rendered (#396; bucket PDF paths keep `TableXpress`) · `&` → `+` sitewide (#395; exempt: 2 VP-title attributions, "E&I Cooperative Services", "A&D", ops/portal) · **"+*" `MoreMarker` on every rendered laminate count** (#390; now `src/components/marketing/MoreMarker.tsx`, PORTALED + fixed + viewport-clamped — extend it, never re-roll; SpeX "see all laminates →" dropped its number; survivors: SpeX StatStrip "66 Laminates" + browse "106 finishes" pending Brian's powder-floor ruling) · knife-edge band on the front face (#391, `EdgeCrossSection` + SpeX `EdgeGlyph`) · Brian's verbatim copy: warranty quote "…every TableX table." (#316), story ¶1 "Based in the Midwest, we've…" (#317, supersedes the 8/26 family-owned rewrite), Team glance rows "Family owned and operated / Not hiring at this time" (#175) · home powder fold: count gone, iron chip → 5th + bordered (#298/#299) · Stretch `inUseImagePos` (#388) · SpeX pill "configure your table →" (#393) · public-spaces "What works here" = Primary-PublicSpaces-01 crop — LAST spaces greybox closed (#311) · Traceless velvet chips wired via `CHIP_OVERRIDE` (Pressed Linen = only chip gap) · compare sticky bar offsets `--header-h`.
- **Full-sheet audit (#394/#145 answered on the cards):** 63/66 sheets are Wilsonart's own PDP renders; 15 solids flat by nature; **4 linens flat because Wilsonart's fullsheet render IS flat at sheet scale** (re-pulled 9/04 — the `_8x8` sample photos carry the weave; converted + stashed at `~/Documents/Clear ph/Clients/TableX/Assets/Laminate Sheets/linen-8x8-samples-2026-09-04/`, NOT wired: the same file skins the SpeX 3D top at 60″ scale). Danny decision: hover-only `sampleUrl`? · 3 missing (Nordic Linen; 2 Formica) = rep/scan ask.
- **Ruling settled (Danny 9/04): Made in America is KEPT** — footer seal + `/about/made-in-america` live (`d1b97b6`, Brian 9/01); the Family-Owned pivot is history. Standing ruling below rewritten.
- **Gotchas:** compare renders EMPTY on plain GET (drive the picker) · Playwright MCP tab is shared across concurrent agents (atomic `run_code` per check) · Workflow `isolation:'worktree'` worktrees the CWD repo (dashboard), so agents make their own tablex-site worktrees + symlink `node_modules` · zsh eats unquoted `--include=*.tsx` · no `timeout` binary on macOS · React 19 lint: no setState in effect bodies (open/close in handlers, listeners in effects).
- **Feedback widget "missing status dropdown" on tablex-site = identity, not code (9/04):** loader/iframe are remote and always current; the popover status control is staff-or-creator only and keys on the token EMAIL. Danny's `digital@tablex.com` login was a non-staff wp_bridge user → `FEEDBACK_IDENTITY_ALIASES` in `src/app/api/feedback/token/route.ts` maps it to `danny@clearph.com` (staff member). ClearPH people only. Memory [[project-feedback-widget-tablex-site]].
- **Local prod server may still be on :3000** (`next start`, build of `58c6e5f`) for Danny's visual pass — kill when done.

## Current state (2026-09-02 PM) — DAY 2 DONE: REP-FOR-DEALER QUOTING SHIPPED · DEMO WENT WELL · FINDINGS PAGE LIVE (tablex-site `main`=`e779911` pushed · dashboard `0e01242` pushed · clearph-feedback `7b61673` live)

**Read memory [[project-brian-live-session-2026-09-01]] first. The work list is now the dashboard page `/project/rep-demo`** (28 items from the Fireflies transcript, collapsible, check-off + notes, realtime; migration **028** `meeting_finding_checks`/`meeting_finding_notes` keyed `<meeting>:<item>` for reuse; 12 items carry a change-order mark). Vault log `2026-09-02-tablex-quoting-dashboard-brian-live-session-day-two-rep-demo`.

- **Rep quoting (`8402c83`):** a rep's draft has no org until submit; `/rep/quotes/[id]` own-quote rail with a required "Submit for dealer" select (strict CRM coverage via NEW `src/lib/portal/rep-coverage.ts`); quote takes the dealer's org + current tier; dealer org members may accept rep-submitted quotes; desk labels "Submitted by (rep)". Seeds: rep = `danny+rep@clearph.com` / `tablex-rep-2026!`, covers Carmel Test Dealer + Bloomington + Business Furniture LLC. + Capitol Business Interiors. TX-2026-0007 = rep test quote.
- **Biggest demo ruling (R-1, NOT built): remove the TableX approval step** — quotes auto-price at the dealer's tier and go straight to the dealer. Freight rules (lift gate, oversized skid) must be real line items first. Danny promised Adam F-1 (line editing in revising) "right after this call".
- **Also shipped:** finish inspector follows light/dark (90% white scrim) + laminate hover race fix (`killTweensOf`; delayed tweens evade `overwrite:"auto"`) · quote PDF TABLE X vector wordmark + new-tab download · home stack cards (Pull-Up / Height Adjustable eyebrows, scrim, bigger name pinned in the `--peek` strip, `--peek-base` keeps card height, `headlineLines` overrides) · `marketedSeries` alphabetical (supersedes 8/28) · browse default `sort=az`.
- **Feedback embed:** silent Path B probe — anyone signed into feedback.clearph.com sees the widget on any site running the script, no site login; opt-in `?cph_feedback=1` sticky per browser, `=0` clears. Safari/Brave miss the probe (partitioned cookies).
- **3D:** Valley Trig STEPs (`3D Files/fwstpfiles`) are redundant with Neal's set — don't swap (HA files 3–4× heavier); ask Valley for 36-deep U-base. Primary corner-mount legs (`fwcornermountingplate`, 4 STEPs, raw GLBs beside them) are legit — NOT on Neal's list. Neal 13-model email SENT 9/02 EOD (thread `19fa41e0e8bdd418`). Rep UI flow verified good by Danny.
- **Open:** Brian D-1 (single-% discount?) / D-2 (Luxe placeholder rule) / D-3 (dark library default) · Kayla+Brian on Safari need the hatch · demo-mode auth revert post-launch.

## Current state (2026-09-02 AM) — LIVE SESSION W/ BRIAN, DAY 1 DONE: ALL SHIPPED (tablex-site `main`=`84914b7`, deploy ● Ready) · WIDGET OFF FOR DEMO

**Brian Craig on-site 9/01–9/03; Danny resumes with "Let's start on day 2 live with brian and team" → one-line greeting, same cadence. Read memory [[project-brian-live-session-2026-09-01]] FIRST.** Four batches on 9/01 (vault logs `2026-09-01-…-batch-one` → `-batch-four-finishes-heroes-shipped`): typography experiment, SpeX base-style filter vs price book, About glance band, careers deleted, marquee home-only, finishes interaction layer + four photo heroes. `ae2583b` = 33 commits shipped, `84914b7` = **feedback widget kill switch OFF for the 9/02 client + sales-rep demo** (`FEEDBACK_WIDGET_ENABLED` in `src/components/FeedbackWidget.tsx` — flip back + push after).

- **/finishes/laminates + /powder-coats** now share `FinishInspector.tsx` (GSAP fly-in click-to-inspect) + `library-controls.tsx` (Ember light/dark ThemeSwitch + "+*" MoreMarker tooltip) + sticky filter bars; laminate chips hover-morph diamond→square (1.15×) and open 1600px masters (`laminates/2k/`, from Drive "Laminate Chips" 2048px). **No hi-res powder scans exist** (300px legacy only). `PowderSpectrum.tsx` = ambient ripple/sweep/shuffle grid on the powder About fold.
- **Photo heroes** (hub mechanism, SiteHeader overlay list): laminates = Trig XL crop · solid-surface = SolidSurfaceDetail3 · custom = CustomLogoTable2 **cropped to exclude third-party marks** · /spaces/cafe-hospitality = ButcherBlock3 via new `SpaceEntry.heroImage`. Powder-coats still opens on a Canvas title block.
- **Demo-mode prod auth (revert/decide post-demo):** seeded dealer is now `danny+dealer@clearph.com` (pw unchanged) · `digital@tablex.com` = new admin · QA rep account has pw `tablex-qa-2026!`.
- **Gotchas:** Danny polls Vercel deploys HIMSELF (never loop) · claude-in-chrome tab freezes GSAP/CSS transitions (verify via inline styles) · Tailwind data-variants on server-rendered wrappers don't get generated from a client toggle — set inline styles · `role="dialog"` probes match the SiteHeader mobile menu · scripts needing `@supabase/supabase-js` must live inside the repo for tsx resolution.
- **Open rulings:** powder "+*" custom-colors claim (Brian) · custom-graphics marks sign-off · "+*" on compare/series/browse counts · 4 other-content marquees · "Openings now" row post-careers · Neal + Byrne drafts + Dekko re-route still unsent.
