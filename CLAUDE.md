...
**Repos:** tablex-site `main` = `c917b5a` (prod ● live-verified, auto-deploys from `main`). Dashboard `main` = `595bf81` (Vercel hub `danny@clearph.com`, repo `github.com/dbreck/tablex-quoting-dashboard`).
...
- Danny rulings wanted (small, from 7/12–13 run reports): ember-hover family unification (marketing saddle vs ops `--color-ember-deep` vs comp `#DC5A18`) · ops pass-3 features (top-nav vs sidebar; org Edit / desk-side New quote / org ⋯ — comps assume them) · fix Fusion Maple tier label in the claude.ai/design finishes project (comp says Core, catalog says Select — app renders correct data) · same-class fixes in the "TableX Products Redesign" design project: Trig headline curly-quote glyphs (comp singles vs catalog doubles — app renders catalog) + collections band wraps 7+1 at 1920 (flag if 8-across intended).
