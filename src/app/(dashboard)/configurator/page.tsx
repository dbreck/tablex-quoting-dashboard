...
export default function ConfiguratorPage() {
  ...
  // Filtered options based on model availability map. Selections default to
  // the first available option by DERIVING an "effective" value during render
  // (`shape || availableShapes[0]`) rather than syncing it in with setState —
  // the latter trips React 19's set-state-in-effect lint and flashes an empty
  // frame before the effect fires. Raw state stays "" until the user picks;
  // the effective value is what every reader (viewer, selects, quote) uses.
