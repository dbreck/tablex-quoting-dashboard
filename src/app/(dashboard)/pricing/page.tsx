...
export default function PricingPage() {
  ...
  const selectedRow = useMemo(
    () => profitData.find((r: ProfitRow) => r.sku === selectedSku) || profitData[0],
    [selectedSku]
  );
  ...
  const gpmValues = useMemo(
    () => profitData.map((r: ProfitRow) => r.gpm * 100).filter((v: number) => v > 0 && v < 100),
    []
  );
          ...
          <div className="mt-4 flex items-center gap-6">
            <div>
              ...
              <p className="text-lg font-bold text-brand-green">
                {(gpmValues.reduce((a: number, b: number) => a + b, 0) / gpmValues.length).toFixed(1)}%
              </p>
            </div>
            <div>
              <p className="text-xs text-slate-500">Median GPM</p>
              <p className="text-lg font-bold text-slate-900">
                {gpmValues.sort((a: number, b: number) => a - b)[Math.floor(gpmValues.length / 2)]?.toFixed(1)}%
              </p>
            </div>
            <div>
